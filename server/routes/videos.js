...
// So aceitamos ate MAX_TVS dispositivos distintos ao mesmo tempo.
router.get('/player/videos', async (req, res) => {
  ...
  const result = await withDb((data) => {
    ...
    let device = data.devices.find((d) => d.id === deviceId);
    if (!device) {
      if (data.devices.length >= maxTvs) {
        return { allowed: false };
      }
      device = { id: deviceId, firstSeenAt: now, lastSeenAt: now };
      data.devices.push(device);
    } else {
      device.lastSeenAt = now;
    }
    return { allowed: true, videos: sortByOrder(data.videos.filter((v) => v.active)) };
  });
